...
def add_product(list, **kwargs):
    if len(list) == 0:
            kwargs['_id'] = len(list) + 1
            list.append(kwargs)
            return kwargs        
    kwargs['_id'] = len(list) + 52
    list.append(kwargs)
    return kwargs
...
def menu_report():
    ...
    for product in products:
        ...
        if product_type in product_type_count:
            product_type_count[product_type] += 1
        else:
            product_type_count[product_type] = 1
    average_price = total_price/count
    formated_average_price = f"{average_price:.2f}"
    if formated_average_price[-1] == '0': 
        formated_average_price = formated_average_price[:-1]

    most_common_type = max(product_type_count, key=product_type_count.get)
    return_string = f'Products Count: {count} - '\
                    f'Average Price: ${formated_average_price} - '\
